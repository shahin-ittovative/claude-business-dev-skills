...
# | 1. Discovery       | pre-BRD | Is this worth building?           | Strategic framework set + go / no-go verdict      |

# | 2. Requirements    | BRD     | What are we building, and why?    | Functional + non-functional requirements          |
...
# \- \*\*Stable two-digit numbering.\*\* Files use `NN-kebab-name.md` so ordering is deterministic and cross-references stay valid.
...
# \*\*claude.ai (web or desktop):\*\* upload each skill through Settings, under Capabilities or Customize then Skills. Each skill must be a folder containing its `SKILL.md` (plus any reference files), zipped and uploaded individually, then toggled on. Custom skills are private to your account; on Team or Enterprise plans an owner can optionally share them org-wide. Note the 1024-character cap on the `description` frontmatter field: any skill whose description runs longer is rejected on upload, so keep trigger-rich detail in the body.

# 

# \*\*Claude Code (CLI):\*\* install at user scope by extracting the skill into:
...
# List installed skills with `/skills` inside a session, or `dir $env:USERPROFILE\\.claude\\skills` in PowerShell outside one. Common failure causes if a skill does not appear: the session was not reloaded, Windows extraction created a double-nested folder, or the `SKILL.md` frontmatter is invalid. Two caveats for CLI use: the Miro MCP must be configured separately via `claude mcp add`, and any output paths that reference `/mnt/user-data/outputs/` are sandbox-specific and may need adjusting.
