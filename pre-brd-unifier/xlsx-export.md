...
1. **Build the payload from all 22 chunks.** Any sheet you leave out of the payload comes out blank in the workbook. To produce a complete pre-BRD, map every framework's answers - not just the analytical ones.
   - **Markdown-only content, never mapped to Excel:** the Investor Assessment (chunk 23) and the **Go-To-Market Strategy** section of chunk 21 have no sheet or whitelisted cells in the reference workbook. Excel covers only the 22 frameworks; chunk 21 maps only its roadmap table to `Roadmap & project plan`. Do not attempt to map investor or GTM cells - an unknown cell/sheet raises `ExportError`. They remain in the Markdown deliverable.
